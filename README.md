...
书架和单本书目录均使用纯文字布局，不生成封面图片或仿书封装饰，也不预留封面列。
...
旧配置中的 `palette` 和 `cover` 不再参与生成，所有作品统一使用无封面的文字目录。
...
欢迎首页加入泳装史尔特尔（`350_surtr_summer#9`）基建小人，桌面端空闲时自动加载；桌面端节省流量或减少动态效果模式下，点击“召唤史尔特尔”加载。手机端禁用：识别 Android、iPhone、iPad（含桌面式标识），并结合窄屏、触屏条件，在创建小人和召唤按钮前直接停止初始化，不启动播放器库、模型或语音加载。初次打开为窄屏的电脑窗口也跳过初始化，放大后需刷新。小人脚本和样式带资源版本号，避免新页面继续复用旧版本缓存。桌面窗口切换到上述条件时会隐藏小人、停止动画并中断模型和语音下载。加载后默认播放，可拖动位置、点击互动和收起；位置和收起状态保存在当前浏览器。其他页面隐藏，切到后台停止动画。
...
小人在页面底部的安全区域随机行走、坐下或躺下，行走碰到左右边界会转向。拖动后松手会受模拟重力影响下落，落地后恢复待机；底部为音乐播放器预留空间。目前不包含侧栏、卡片顶部的平台碰撞。点击小人会打断当前活动并执行互动动作，结束后恢复待机。下方“走一走 / 坐下 / 躺下”按钮默认隐藏，鼠标悬停在小人区域时显示；支持的触屏电脑上轻点小人展开，再次轻点或点击外部收起，键盘聚焦小人也可显示。缺失的模型动作会禁用对应按钮。离开首页或切到后台时，动画、重力和行为计时一起停止。

小人模型先加载并显示，显示后等待 3 秒，再在浏览器空闲时下载、解码日文语音。语音开关在准备完成前隐藏，完成后才加入原有的悬停菜单；准备期间戳一戳只做动作，不会在加载完后补播。关闭小人、离开首页或切到后台会取消尚未开始的语音任务和进行中的下载，返回后继续准备；加载失败时保持按钮隐藏，再戳一下可重新安排加载。

准备好后，戳一戳随机播放史尔特尔日文语音，包含交谈 1–3、闲置、戳一下、信赖触摸、问候七句，避免连续重复。连续点击会停止上一句，只播放最新一次；拖动、自动活动和动作按钮不触发语音。顶部名称、语音开关和关闭按钮与下方动作按钮一起默认隐藏，悬停、触屏电脑轻点展开或键盘聚焦时显示。“语音开 / 已静音”开关记住访客选择，关闭小人、切页或切到后台都会停止语音。

语音来自 Ark-Voice 固定版本，原始 OGG 和分句索引保存在 `source/models/surtr-summer/voice-jp.ogg`、`voice-jp.json`。浏览器使用 OfflineAudioContext 预先解码并只缓存选中的片段，实际音频输出在用户点击时才启用；来源及版权见同目录 `CREDITS.txt`。若浏览器不支持该 OGG 编码或加载失败，语音开关保持隐藏，控制台记录原因，动画仍可使用。

手动查看：重启 `hexo s` 后打开首页；将小人拖到高处松手，观察落地；尝试悬停显示动作按钮和戳一戳，确认手机端不显示小人及召唤按钮，并检查语音单句播放、连续点击、静音记忆、切页停止、窗口缩放以及关闭后重新召唤。此功能尚未运行构建或浏览器验证。
